...
function App(){
  ...
  const factualPrompt=(item)=>{
    ...
    return {question:'Explique précisément cette actualité : que s’est-il passé, qui est concerné et pourquoi est-ce important ?',answer:item.description||item.title};
  };
    ...
    <main>
      ...
      {tab==='Tests fictifs'&&<section className="noTop">
        ...
        {deck.length>0&&deckIndex<deck.length&&(()=>{const c=deck[deckIndex];return <article className="examCard flashExam">
          ...
          {c.kind==='photo'?<>
            {c.item.image?<img className="testPhoto" src={c.item.image} alt="Photo liée à l’actualité"/>:<div className="fakePhoto"><span>PHOTO D’ACTUALITÉ</span><div>{c.item.category}</div></div>}
